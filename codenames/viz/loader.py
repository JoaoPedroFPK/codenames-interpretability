...
def sample_ids(ids, n: int, seed: int = 2026) -> List[int]:
...
def sample_boards(index: pd.DataFrame, n: int, seed: int = 2026) -> List[int]:
...
def giver_value_map(general: pd.DataFrame, row_id: int) -> Dict[str, str]:
    """Map each giver-feature tag to the board's actual value.

    The vector index stores social rows by their tag (e.g. ``giver.country``),
    but the model saw the *value* (``united states``). This returns
    ``{"giver.country": "united states", ...}`` so figures can relabel social
    rows/points with the true value rather than the bare tag.
    """
    meta = board_meta(general, row_id)
    gf = meta.get("giver_features")
    if not isinstance(gf, dict):
        return {}
    return {str(k): str(v) for k, v in gf.items()}
