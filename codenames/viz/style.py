...
# Word-type styling shared by projection markers and heatmap axis-label
# colours. ``order`` fixes legend order and z-order. Muted, paper-like palette:
...
# Colour + marker + size + legend label + ordering for both projection markers
# and heatmap axis-label colours. Shapes are kept varied (D / o / X / ^) so
# type stays distinguishable; the neutral dot is
...
# Text rendered in a word-type colour needs more contrast than a marker: the
# pale tan that works as a receding dot is illegible as 5–6 pt text on white.
# Markers keep the light hue; any TEXT (word labels, axis tick labels) uses
# this darkened variant instead.
TEXT_TAN = "#7a6a45"
...
def label_color(word_type: str) -> str:
    """Colour for TEXT in a word-type hue (darkened where the hue is pale)."""
    if word_type == "tan":
        return TEXT_TAN
    return style_for(word_type)["color"]
...
def fig_header(fig, title: str, subtitle: str = None, *,
               title_size: float = None, subtitle_size: float = None,
               title_y: float = 0.99, subtitle_y: float = 0.955) -> None:
    """One header treatment for every board figure (heatmap AND projection):
    a bold human title (model, semantics) over a smaller grey provenance line
    (board id, layer/condition) — so the two figure families read the same.
    """
    fig.suptitle(title, fontsize=title_size or FS["suptitle"],
                 fontweight="bold", y=title_y)
    if subtitle:
        fig.text(0.5, subtitle_y, subtitle, ha="center", va="top",
                 fontsize=subtitle_size or FS["footnote"] + 1,
                 color=FOOTNOTE_COLOR)


# === Model display names ==================================================
# Human names for figure headers, keyed by file prefix — never show internal
# identifiers like "t5_outputs" or "random_qwen" on a rendered figure.
MODEL_DISPLAY: Dict[str, str] = {
    "mistral": "Mistral",
    "qwen": "Qwen",
    "random_qwen": "Qwen (random init)",
    "bert": "BERT",
    "random_bert": "BERT (random init)",
    "t5": "T5",
...
def display_name(prefix: str) -> str:
    """Display name for a model prefix (falls back to a cleaned prefix)."""
    return MODEL_DISPLAY.get(prefix, prefix.replace("_", " ").title())
