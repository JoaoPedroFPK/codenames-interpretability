...
_REF_FS = {"title": 10, "axis": 8, "tick": 7, "label": 9, "suptitle": 12,
           "legend": 8, "footnote": 7}

_REF_DISCLAIMER = (
    "Coordinates produced by UMAP projection (n_components=2, metric=cosine, "
    "random_state=2026). No quantitative metric is derived from projected "
...
def _ref_text_color(wt: str) -> str:
    """Text colour for a word label: dark enough to read in a printed PDF.

    Marker hues that are too pale as small text are swapped for darker
    variants when used for LABELS (markers keep the light hue).
    """
    from .style import TEXT_TAN

    dark = {"tan": TEXT_TAN, "giver_feature": "#48637a"}
    return dark.get(wt, _ref_color(wt))
...
def reduce(vectors: np.ndarray, method: str, seed: int = 2026) -> np.ndarray:
...
def embed_all(
    vectors: np.ndarray, k: int = 5, seed: int = 2026,
...
def _draw_panel(ax, emb, words, word_types, vectors, *, layer, num_layers,
    ...
    # Labels: every word; hint and target bold; target tagged "[T]". Pale hues
    # are darkened for text, and every label gets a paper-coloured halo so it
    # stays readable over markers, arrows and leader lines in the printed PDF.
    import matplotlib.patheffects as path_effects

    halo = [path_effects.withStroke(linewidth=1.8, foreground=_BG_COLOR)]
    texts = []
    for i, wt in enumerate(word_types):
        ...
        texts.append(ax.text(
            ...
            fontweight=weight, color=_ref_text_color(wt), zorder=5,
            path_effects=halo,
        ))
...
def plot_layer_panels(
    ...
    *,
    ...
    with plt.rc_context(_REF_RC):
        ...
        # Shared header mechanism (style.fig_header, same as the heatmap):
        # bold suptitle (the human story: model, clue → targets) over the
        # provenance (board id, condition) as a smaller grey line, then the
        # framed corner legend and italic disclaimer footnote.
        if title:
            fig_header(fig, title, subtitle,
                       title_size=_REF_FS["suptitle"],
                       subtitle_size=_REF_FS["footnote"] + 1,
                       subtitle_y=0.962)
        ...
        fig.tight_layout(rect=(0, 0.04, 1, 0.945 if subtitle else 0.96))
