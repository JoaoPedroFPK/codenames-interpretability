...
Instantiation uses ``accelerate.init_empty_weights()`` + ``to_empty(device)``
+ ``model.apply(model._init_weights)``, followed by an explicit RMSNorm
re-init loop: Qwen2's ``_init_weights`` does not handle RMSNorm, and the
...
weight-independent). The seed is re-set immediately before construction so
the resulting weights are deterministic.
"""
...
def load_qwen_random(random_seed: int = 2026) -> Tuple[Any, Any, Dict[str, Any]]:
