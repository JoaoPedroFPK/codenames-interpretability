...
The tokenizer is loaded from the pretrained bert-base-uncased checkpoint
(tokenization is weight-independent); the model is constructed via
``BertModel(BertConfig(output_hidden_states=True))`` so no pretrained weights
are loaded. An embedding-std check raises if the weights look pretrained.
"""
...
def load_bert_random(random_seed: int = 2026) -> Tuple[Any, Any, Dict[str, Any]]:
