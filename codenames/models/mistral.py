...
Loaded in fp16 with no device_map; hidden states are requested at inference
time. ``attn_implementation`` defaults to the HuggingFace default for the
model class; pass ``"flash_attention_2"`` to opt into FA2 when ``flash_attn``
is available.
"""
