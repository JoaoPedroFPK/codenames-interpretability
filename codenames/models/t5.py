...
T5EncoderModel only (no decoder); fp32 (T5's native precision — fp16 causes
overflow in some T5 layers). ``output_hidden_states=True`` is set at load
time on the model config.
