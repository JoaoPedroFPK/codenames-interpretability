...
Iterates over the sampled boards under both prompt conditions, calls
:func:`extraction.run_instance` per board, and flushes results in shards.
Configuration (``MODEL_PREFIX``, ``BASE_DIR``, ``SAMPLE_SIZE``, etc.) comes
from the ``Contract`` and the per-model metadata; generation is enabled by
passing a ``generation_fn``.
"""
...
def run_extraction(
    *,
    ...
    for mode_flag in experiment_modes:
        ...
        # --- Resume reconciliation (driven by the manifest) ---
        if resume:
