...
The ``instruction_body`` is identical across all seven models; only the
per-model chat-template wrapping differs. Three strategies are supported:
...
def build_instruction_body(
    ...
    """Construct the shared instruction body used by all models.

    Returns the body plus a dict mapping each giver-feature column key to the
    exact substring written into the prompt; span detection uses this to locate
    the feature spans afterwards.
    """
    words_block = "\n".join(
...
def build_prompt(
    ...
    - ``"mistral_inst"``: wraps the body as ``<s>[INST] {body} [/INST]``. The
      tokenizer argument is unused for this strategy but kept in the signature
      for uniformity.
    - ``"chatml"``: builds a ``[{"role": "system", ...}, {"role": "user", ...}]``
      message list with the system message
