...
These values are fixed by the methodology and shared by every model. The
default instance ``CONTRACT_V1`` is frozen so a run cannot mutate it.
Model-specific values (model name, prefix, output directory, max sequence
length) are not in the contract; they live in the model loaders.

``Acceleration`` is a separate frozen dataclass holding implementation choices
that trade exact numerical agreement with the reference path for wall-clock
speedup; these are not part of the methodology. ``ACCEL_REFERENCE`` is the
all-defaults (reference) instance.
"""
...
class Contract:
    """Frozen experimental parameters. See Chapter 5 of the thesis for rationale.

    Attributes
    ----------
    sample_size
        Number of Codenames Duet turns drawn from CULTURAL CODES per run
        (with ``random_state=random_seed``). Thesis §5.3.
    candidate_order
    ...
    n_shuffles
        ...
        the canonical (permutation_id=0) ordering. Thesis §5.6.2.
...
@dataclass(frozen=True)
class Acceleration:
    """Acceleration flags. None of these change the methodology; each trades
    exact numerical agreement with the reference path for wall-clock speedup.
    The ``compare`` CLI subcommand quantifies the per-cell delta a given
