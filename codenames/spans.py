...
Architecture-agnostic. The cosine and pooling functions return ``np.float16``
arrays, and ``cosine_similarity_np`` returns 0.0 (not NaN) on zero-norm
vectors; downstream metrics depend on both behaviours.
"""
...
def cosine_similarity_np(a: np.ndarray, b: np.ndarray) -> float:
    ...
    (not NaN); downstream metrics depend on this.
    """
    na = np.linalg.norm(a)
    nb = np.linalg.norm(b)
    if na == 0.0 or nb == 0.0:
